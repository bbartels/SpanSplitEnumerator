...
namespace SpanSplit
{
    public ref struct BufferedSplitEnumerator
    {
        ...
        private int _buffCurrent;

        public BufferedSplitEnumerator GetEnumerator() => this;

        public Range Current => new Range(_current, _endCurrent);
        public ReadOnlySpan<int> Buffer => _buff.Slice(0, _buffCount);

        public BufferedSplitEnumerator(ReadOnlySpan<char> span, char separator, Span<int> buffer)
        {
            _isInitialized = true;
            _buffer = span;
        ...
        {
            if (!_isInitialized || ( _endCurrent >= _buffer.Length && _buffCurrent == 0))
            {
                return false;
            }

            _current = _endCurrent + 1;

            if (_startNext < _buffer.Length && _buffCurrent == 0) {
                Test();
            }

            _endCurrent = 0 == _buffCurrent ? _buffer.Length : _buff[_buffCount - _buffCurrent--];
        ...
        private void Test()
        {
            ReadOnlySpan<char> slice = _buffer.Slice(_startNext);

            (_buffCount, int charIndex) = MakeSeparatorListVectorized(slice, _separator, _buff, _startNext);
            _startNext = charIndex + _startNext + 1;
            _buffCurrent = _buffCount;
        }

        // Problems
        // Generic type, vector size, size struct
        // Lazy or greedy
